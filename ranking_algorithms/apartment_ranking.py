...
class SantaCruzApartmentRanking(BaseRankingAlgorithm):
    ...
    def _get_console_columns(self) -> Dict[str, Dict]:
        """Get apartment-specific console columns."""
        return {
            'title': {'header': 'Title', 'width': 35},
            'score': {'header': 'Score', 'align': 'center'},
            'price': {'header': 'Price', 'align': 'right'},
            'distance': {'header': 'Dist(mi)', 'align': 'right'},
            'sqr_foot': {'header': 'SqFt', 'align': 'right'},
            'url': {'header': 'URL', 'width': 40}
        }
    
    def _format_console_value(self, listing: Dict, col_key: str, col_info: Dict) -> str:
        """Format apartment-specific console values."""
        if col_key == 'distance':
            distance = listing.get('distance', -1)
            return f"{distance:.1f}" if distance > 0 else "N/A"
        
        elif col_key == 'sqr_foot':
            sqr_foot = listing.get('sqr_foot', -1)
            return f"{sqr_foot:,}" if sqr_foot > 0 else "N/A"
