...
class AdParser:
    ...
    @property
    def coord(self) -> str:
        text_coords = self.soup.find('meta', attrs={'name': 'geo.position'})
        if text_coords:
            return text_coords['content']
        else:
            return ""

    @property
    def price(self) -> float:
        price_span = self.soup.find("span", class_ = "price")
        if price_span is not None:
            return format_price(price_span.text)
        else:
    ...
    def attributes(self) -> Dict:
        ...
        for attr_group in self.soup.find_all("div", class_ = "attrgroup"):
    ...
    def address(self) -> str:
        addr = self.soup.find("h2", class_ = "street-address")
        if addr is not None:
            return addr.text
        else:
            return ""
